...
def d22p2(file) -> int:
    def make_edge(rows, ps, px, py, cx, cy, sx, sy, dx, dy, dm):
        x_coord = (px[0] * ps + max(0, (cx[0] * ps) - 1), px[1] * ps + max(0, (cx[1] * ps) - 1))
        y_coord = (py[0] * ps + max(0, (cy[0] * ps) - 1), py[1] * ps + max(0, (cy[1] * ps) - 1))
    ...
    if 'b' not in file:
        ...
        # A to D
        make_edge(rows, size, A, D, top_left, bot_left, Node.set_l, Node.set_l, -1j, 1j, -1)
            
        # A to F
        make_edge(rows, size, A, F, top_left, top_left, Node.set_u, Node.set_l, 1, -1j, -1j)
        
        # B to F
        make_edge(rows, size, B, F, top_left, bot_left, Node.set_u, Node.set_d, 1, 1, 1)
        
        # B to E
        make_edge(rows, size, B, E, top_right, bot_right, Node.set_r, Node.set_r, -1j, 1j, -1)
        
        # B to C
        make_edge(rows, size, B, C, bot_left, top_right, Node.set_d, Node.set_r, 1, -1j, -1j)
        
        # C to D
        make_edge(rows, size, C, D, top_left, top_left, Node.set_l, Node.set_u, -1j, 1, 1j)
        
        # E to F
        make_edge(rows, size, E, F, bot_left, top_right, Node.set_d, Node.set_r, 1, -1j, -1j)
    else:
        size = 4
        A = (0, 2)
        B = (1, 0)
        ...
        # A to B
        make_edge(rows, size, A, B, top_left, top_right, Node.set_u, Node.set_u, 1, -1, -1)
            
        # A to C
        make_edge(rows, size, A, C, top_left, top_left, Node.set_l, Node.set_u, -1j, 1, 1j)
        
        # A to F
        make_edge(rows, size, A, F, top_right, bot_right, Node.set_r, Node.set_r, -1j, 1j, -1)
        
        # B to E
        make_edge(rows, size, B, E, bot_left, bot_right, Node.set_d, Node.set_d, 1, -1, -1)
        
        # B to F
        make_edge(rows, size, B, F, top_left, bot_right, Node.set_l, Node.set_d, -1j, -1, -1j)
        
        # C to E
        make_edge(rows, size, C, E, bot_left, bot_left, Node.set_d, Node.set_l, 1, 1j, 1j)
        
        # D to F
        make_edge(rows, size, D, F, bot_right, top_left, Node.set_r, Node.set_u, 1j, 1, -1j)
